<template>
    ...
    <ResourceTableLayout
        :table="table"
        :columns="columns"
        :search-placeholder="$t('Search student')"
        create-permission="create students"
        create-route-name="academics.settings.students.create"
        :create-label="$t('Add Student')"
        filter-display="row"
...
const table = useSettingsTable({
    endpoint: '/academics/settings/students',
...
const actions = useRowActions({
    editRouteName: 'academics.settings.students.edit',
    deleteEndpoint: '/academics/settings/students/:id/destroy',
    onDeleteSuccess: () => {
        table.refresh();
    },
    messages: {
        successMessage: $t('Student deleted successfully.'),
        errorMessage: $t('An error occurred while deleting the student.'),
        confirmMessage: $t('Are you sure you want to delete this student?'),
    }
});
